...
public class Solution {
	...
	boolean[] visited;
	int ret = 0;
	private void dfs(int vertex, List<Integer> path, int end, int noOfValidLocations) {
		if(vertex == end && path.size() == noOfValidLocations) {
		...
		for(int v : adjList.get(vertex)) {
			if(!visited[v]) {
				...
				newPath.add(v);
				dfs(v, newPath, end, noOfValidLocations);
			}
		}
	...
	public int uniquePathsIII(int[][] grid) {
		...
		path.add(start);
		dfs(start, path, end, noOfValidLocations);
		return ret;
    }
